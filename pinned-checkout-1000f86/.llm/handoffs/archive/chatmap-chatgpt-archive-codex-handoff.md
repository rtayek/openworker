...
3. Locate the exported conversation data, normally `conversations.json`.
...
If this export differs materially from the expected ChatGPT export structure, stop and report the differences before designing around guesses.

## Required Import Behavior

Add a bulk ChatGPT archive importer that:

1. Reads `conversations.json` directly from the ZIP.
2. Imports all usable conversations, not merely the first one.
...
- missing `conversations.json` produces a clear error
